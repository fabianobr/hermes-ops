...
- Hermes Agent is installed on the host for user `<operator>`.
- Host launcher path: `$HOME/.local/bin/hermes`
- Host state directory: `$HOME/.hermes`
...
- Backed up the previous host config locally. Host backup artifacts are intentionally not committed to this public-safe repository.
...
- Install and continuity notes: [docs/install-notes.md](install-notes.md)
- Baseline model decision: [docs/baseline-decision.md](baseline-decision.md)
- Provider bring-up runbook: [docs/provider-setup.md](provider-setup.md)
- Architecture and security boundaries: [docs/ARCHITECTURE.md](ARCHITECTURE.md)
- Local provider template: [config/hermes-local-providers.yaml.example](../config/hermes-local-providers.yaml.example)
- Backend validation script: [scripts/check_local_backends.py](../scripts/check_local_backends.py)
- Ollama model inventory script: [scripts/list_ollama_models.py](../scripts/list_ollama_models.py)
- Baseline validation script: [scripts/validate_hermes_baseline.sh](../scripts/validate_hermes_baseline.sh)
- Telegram gateway check script: [scripts/check_telegram_gateway.sh](../scripts/check_telegram_gateway.sh)
- Telegram setup runbook: [docs/telegram-setup.md](telegram-setup.md)
- Telegram env template: [config/hermes-telegram.env.example](../config/hermes-telegram.env.example)
- Environment inventory and prerequisite logs are generated locally under `logs/` and ignored by git.

## Verified Commands
Run these from a normal host shell as user `<operator>`:
...
After selecting `qwen3-coder:30b` as the best candidate, Hermes was tested on a small grounded repository task in `<repo>`.
...
- [config/hermes-telegram.env.example](../config/hermes-telegram.env.example)
- [scripts/check_telegram_gateway.sh](../scripts/check_telegram_gateway.sh)
...
- `hermes pairing list` shows one pending Telegram request for user `<telegram-user-id>` / `<telegram-username>`
...
- `hermes pairing list` still shows:
  - one pending Telegram request for `<telegram-user-id>` / `<telegram-username>`
  - no approved users
...
- `ps -ef` shows a live foreground gateway process:
  - PID `1391163`
  - command: `$HOME/.hermes/hermes-agent/venv/bin/python3 $HOME/.hermes/hermes-agent/venv/bin/hermes gateway run -v`
  - terminal: `pts/1`
- user-provided live gateway log confirms:
  - Telegram connected in polling mode
  - gateway running with `1` platform
  - inbound DM processed from `<telegram-username>`
...
- main PID: `23036`
- command: `$HOME/.hermes/hermes-agent/venv/bin/python -m hermes_cli.main gateway run --replace`
...
- [scripts/check_telegram_gateway.sh](../scripts/check_telegram_gateway.sh) now also looks for both manual and systemd gateway process forms

Safety record:

- a local backup was saved before lockout cleanup, but it is intentionally not committed because it can contain Telegram identifiers
...
- store local backups with timestamps in `backups/`, but keep real backup contents out of git
