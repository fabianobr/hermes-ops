...
## Result
- Hermes Agent is installed on the real host for user `<operator>`.
- The launcher exists at `$HOME/.local/bin/hermes`.
- Hermes state directory exists at `$HOME/.hermes`.
...
Validate Hermes directly in a normal host shell for user `<operator>`, then continue here with provider configuration:
...
- Added [scripts/check_local_backends.py](../scripts/check_local_backends.py) to validate LM Studio and Ollama without `curl`.
- Added [config/hermes-local-providers.yaml.example](../config/hermes-local-providers.yaml.example) as a host-side template for local Hermes provider wiring.
- Added [docs/provider-setup.md](provider-setup.md) as the runbook for provider bring-up.
...
- A backup of the pre-change host config was saved locally under `backups/` and is intentionally not committed.
...
- A real workspace-inspection task was tested in `<repo>`.
...
- Added [docs/telegram-setup.md](telegram-setup.md) as the Telegram runbook.
- Added [config/hermes-telegram.env.example](../config/hermes-telegram.env.example) as a host-side secret-free example block for `~/.hermes/.env`.
- Added [scripts/check_telegram_gateway.sh](../scripts/check_telegram_gateway.sh) for a reproducible non-secret validation pass.
...
- `bash ~/AI/hermes-ops/scripts/check_telegram_gateway.sh` now reports:
  ...
  - one pending Telegram pairing request for `<telegram-username>`
...
- Cleared a temporary pairing lockout after repeated failed approval attempts and saved a local backup under `backups/`.
...
- Observed result from `hermes pairing list`:
  - one pending Telegram request for `<telegram-username>`
  - no approved users
...
- A direct process inspection with `ps -ef` confirmed the gateway is actually running in another terminal:
  ...
  - terminal `pts/1`
- The live gateway log from that terminal confirms successful Telegram polling connection and a real DM round-trip from `<telegram-username>`.
...
- Hermes created and enabled:
  ...
  - symlink under `$HOME/.config/systemd/user/default.target.wants/`
...
- Updated [scripts/check_telegram_gateway.sh](../scripts/check_telegram_gateway.sh) so process detection covers both:
