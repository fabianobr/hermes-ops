...
## Host Validation
Run these commands in a normal shell as user `fabiano`:
...
Copy the relevant structure from [config/hermes-local-providers.yaml.example](/home/fabiano/AI/hermes-ops/config/hermes-local-providers.yaml.example) and replace `qwen3-coder:30b` if you want a different Ollama model.

Use [scripts/list_ollama_models.py](/home/fabiano/AI/hermes-ops/scripts/list_ollama_models.py) to identify a model whose reported context window is at least `64000`, which Hermes requires.
